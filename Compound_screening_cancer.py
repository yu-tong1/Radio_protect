...
def match_pathways(file_name1, file_name2, file_name3,file_name4, original_filename):
    ...
    if used_col2 is None:
        ...
        matched1.to_csv(os.path.join(output_dir,"Matching cancer results of {} ingredients.csv".format(base_name)), index=False, encoding='utf-8-sig')
        matched2.to_csv(os.path.join(output_dir,"Unmapped cancer results of {} ingredients.csv".format(base_name)), index=False, encoding='utf-8-sig')
...
def process_tcm_file_cancer(tcm_file_path, original_filename):
    matched, unmatched = match_pathways(tcm_file_path, 'results of target matching cancer.csv', 'herb_ingredient.csv', 'DEGs_cancer.csv', original_filename)
